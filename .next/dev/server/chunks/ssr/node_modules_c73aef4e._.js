...
"[project]/node_modules/lucide-react/dist/esm/icons/flip-horizontal.js [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const FlipHorizontal = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$lucide$2d$react$2f$dist$2f$esm$2f$createLucideIcon$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"])("flip-horizontal", __iconNode);
;
 //# sourceMappingURL=flip-horizontal.js.map
}),
"[project]/node_modules/lucide-react/dist/esm/icons/flip-horizontal.js [app-ssr] (ecmascript) <export default as FlipHorizontal>", ((__turbopack_context__) => {
"use strict";
...
"[project]/node_modules/html2canvas-pro/dist/html2canvas-pro.esm.js [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const parseDocumentSize = (document1)=>{
    const body = document1.body;
    const documentElement = document1.documentElement;
...
const COMMA_TOKEN = {
    type: 4 /* TokenType.COMMA_TOKEN */ 
};
const SUFFIX_MATCH_TOKEN = {
    type: 13 /* TokenType.SUFFIX_MATCH_TOKEN */ 
};
const PREFIX_MATCH_TOKEN = {
    type: 8 /* TokenType.PREFIX_MATCH_TOKEN */ 
};
const COLUMN_TOKEN = {
    type: 21 /* TokenType.COLUMN_TOKEN */ 
};
const DASH_MATCH_TOKEN = {
    type: 9 /* TokenType.DASH_MATCH_TOKEN */ 
};
const INCLUDE_MATCH_TOKEN = {
    type: 10 /* TokenType.INCLUDE_MATCH_TOKEN */ 
};
...
const SUBSTRING_MATCH_TOKEN = {
    type: 14 /* TokenType.SUBSTRING_MATCH_TOKEN */ 
};
const BAD_URL_TOKEN = {
    type: 23 /* TokenType.BAD_URL_TOKEN */ 
};
const BAD_STRING_TOKEN = {
    type: 1 /* TokenType.BAD_STRING_TOKEN */ 
};
const CDO_TOKEN = {
    type: 25 /* TokenType.CDO_TOKEN */ 
};
const CDC_TOKEN = {
    type: 24 /* TokenType.CDC_TOKEN */ 
};
const COLON_TOKEN = {
    type: 26 /* TokenType.COLON_TOKEN */ 
};
const SEMICOLON_TOKEN = {
    type: 27 /* TokenType.SEMICOLON_TOKEN */ 
};
...
class Tokenizer {
        ...
        switch(codePoint){
            ...
            case COMMERCIAL_AT:
                ...
                if (isIdentifierStart(a1, a2, a3)) {
                    const value = this.consumeName();
                    return {
                        type: 7 /* TokenType.AT_KEYWORD_TOKEN */ ,
                        value
                    };
                }
                break;
    ...
    consumeUnicodeRangeToken() {
        ...
        if (questionMarks) {
            ...
            return {
                type: 30 /* TokenType.UNICODE_RANGE_TOKEN */ ,
                start,
                end
            };
        }
    ...
    consumeIdentLikeToken() {
        ...
        if (value.toLowerCase() === 'url' && this.peekCodePoint(0) === LEFT_PARENTHESIS) {
            ...
            this.consumeCodePoint();
            return {
                type: 19 /* TokenType.FUNCTION_TOKEN */ ,
                value
            };
        }
        return {
            type: 20 /* TokenType.IDENT_TOKEN */ ,
            value
        };
    }
    consumeUrlToken() {
...
const parseLengthPercentageTuple = (tokens)=>tokens.length > 1 ? [
        tokens[0],
        tokens[1]
    ] : [
        tokens[0]
...
const extractLabComponents = (args)=>{
    const tokens = args.filter(nonFunctionArgSeparator), l = tokens[0].type === 16 /* TokenType.PERCENTAGE_TOKEN */  ? tokens[0].number / 100 : isNumberToken(tokens[0]) ? tokens[0].number : 0, a = tokens[1].type === 16 /* TokenType.PERCENTAGE_TOKEN */  ? tokens[1].number / 100 : isNumberToken(tokens[1]) ? tokens[1].number : 0, b = isNumberToken(tokens[2]) || isDimensionToken(tokens[2]) ? tokens[2].number : 0, alpha = typeof tokens[4] !== 'undefined' && isLengthPercentage(tokens[4]) ? getAbsoluteValue(tokens[4], 1) : 1;
    return [
        l,
        a,
        b,
...
const extractOkLchComponents = (args)=>{
    const tokens = args.filter(nonFunctionArgSeparator), l = tokens[0].type === 16 /* TokenType.PERCENTAGE_TOKEN */  ? tokens[0].number / 100 : isNumberToken(tokens[0]) ? tokens[0].number : 0, c = tokens[1].type === 16 /* TokenType.PERCENTAGE_TOKEN */  ? tokens[1].number / 100 : isNumberToken(tokens[1]) ? tokens[1].number : 0, h = isNumberToken(tokens[2]) || isDimensionToken(tokens[2]) ? tokens[2].number : 0, a = typeof tokens[4] !== 'undefined' && isLengthPercentage(tokens[4]) ? getAbsoluteValue(tokens[4], 1) : 1;
    return [
        l,
        c,
        h,
 ...
 */ const _color = (context, args)=>{
    ...
    if (is_absolute) {
        ...
        const c1 = isNumberToken(tokens[1]) ? tokens[1].number : 0, c2 = isNumberToken(tokens[2]) ? tokens[2].number : 0, c3 = isNumberToken(tokens[3]) ? tokens[3].number : 0, a = tokens.length > 4 && tokens[4].type === 6 /* TokenType.DELIM_TOKEN */  && tokens[4].value === '/' && isNumberToken(tokens[5]) ? tokens[5].number : 1;
        ...
        const extractComponent = (color, token)=>{
            ...
            const parseCalc = (args)=>{
                ...
                for (const part of parts){
                    expression += part.type === 18 /* TokenType.FUNCTION */  && part.name === 'calc' ? parseCalc(part.values) : isNumberToken(part) ? part.number : part.type === 6 /* TokenType.DELIM_TOKEN */  || isIdentToken(part) ? part.value : '';
                }
                expression += ')';
                return expression;
            };
        ...
        const from_colorspace = tokens[1].type === 18 /* TokenType.FUNCTION */  ? tokens[1].name : isIdentToken(tokens[1]) || tokens[1].type === 5 /* TokenType.HASH_TOKEN */  ? 'rgb' : 'unknown', to_colorspace = isIdentToken(tokens[2]) ? tokens[2].value : 'unknown';
        let from = tokens[1].type === 18 /* TokenType.FUNCTION */  ? tokens[1].values : isIdentToken(tokens[1]) ? [
            tokens[1]
        ] : [];
        ...
        const from_color = fromColorToXyz(context, from), from_final_colorspace = toColorFromXyz(from_color), c1 = extractComponent(from_final_colorspace, tokens[3]), c2 = extractComponent(from_final_colorspace, tokens[4]), c3 = extractComponent(from_final_colorspace, tokens[5]), a = tokens.length > 6 && tokens[6].type === 6 /* TokenType.DELIM_TOKEN */  && tokens[6].value === '/' && isNumberToken(tokens[7]) ? tokens[7].number : 1;
...
function isSupportedImage(value) {
    return !(value.type === 20 /* TokenType.IDENT_TOKEN */  && value.value === 'none') && (value.type !== 18 /* TokenType.FUNCTION */  || !!SUPPORTED_IMAGE_FUNCTIONS[value.name]);
}
...
class CSSParsedDeclaration {
    ...
    isInlineLevel() {
        return contains(this.display, 4 /* DISPLAY.INLINE */ ) || contains(this.display, 33554432 /* DISPLAY.INLINE_BLOCK */ ) || contains(this.display, 268435456 /* DISPLAY.INLINE_FLEX */ ) || contains(this.display, 536870912 /* DISPLAY.INLINE_GRID */ ) || contains(this.display, 67108864 /* DISPLAY.INLINE_LIST_ITEM */ ) || contains(this.display, 134217728 /* DISPLAY.INLINE_TABLE */ );
    }
}
        ...
        case 3 /* PropertyDescriptorParsingType.TYPE_VALUE */ :
            switch(descriptor.format){
                ...
                case 'length-percentage':
                    const value1 = parser.parseComponentValue();
                    return isLengthPercentage(value1) ? value1 : ZERO_LENGTH;
                case 'time':
...
const testRangeBounds = (document1)=>{
    const TEST_HEIGHT = 123;
    if (document1.createRange) {
        const range = document1.createRange();
        if (range.getBoundingClientRect) {
            const testElement = document1.createElement('boundtest');
...
const testIOSLineBreak = (document1)=>{
    const testElement = document1.createElement('boundtest');
...
const testSVG = (document1)=>{
    const img = new Image();
    const canvas = document1.createElement('canvas');
...
const testForeignObject = (document1)=>{
    const canvas = document1.createElement('canvas');
    const size = 100;
    canvas.width = size;
    canvas.height = size;
    ...
    return loadSerializedSVG$1(svg).then((img)=>{
        ...
        ctx.fillStyle = 'red';
        ctx.fillRect(0, 0, size, size);
        const node = document1.createElement('div');
...
class InputElementContainer extends ElementContainer {
    constructor(context, input){
        ...
        if (this.type === CHECKBOX || this.type === RADIO) {
            ...
            this.styles.borderTopStyle = this.styles.borderRightStyle = this.styles.borderBottomStyle = this.styles.borderLeftStyle = 1 /* BORDER_STYLE.SOLID */ ;
            this.styles.backgroundClip = [
                0 /* BACKGROUND_CLIP.BORDER_BOX */ 
            ];
            this.styles.backgroundOrigin = [
                0 /* BACKGROUND_ORIGIN.BORDER_BOX */ 
            ];
...
const createsStackingContext = (styles)=>{
    ...
    return contains(styles.display, 268435456 /* DISPLAY.INLINE_FLEX */ ) || contains(styles.display, 33554432 /* DISPLAY.INLINE_BLOCK */ ) || contains(styles.display, 536870912 /* DISPLAY.INLINE_GRID */ ) || contains(styles.display, 134217728 /* DISPLAY.INLINE_TABLE */ );
};
...
class DocumentCloner {
    ...
    createCustomElementClone(node) {
        ...
        if (("TURBOPACK compile-time value", "undefined") !== 'undefined' && !customElements.get('html2canvascustomelement')) //TURBOPACK unreachable
        ;
    ...
    cloneNode(node, copyStyles) {
        ...
        const window1 = node.ownerDocument.defaultView;
        if (window1 && isElementNode(node) && (isHTMLElementNode(node) || isSVGElementNode(node))) {
            const clone = this.createElementClone(node);
            clone.style.transitionProperty = 'none';
            const style = window1.getComputedStyle(node);
            const styleBefore = window1.getComputedStyle(node, ':before');
            const styleAfter = window1.getComputedStyle(node, ':after');
    ...
    resolvePseudoContent(node, clone, style, pseudoElt) {
        ...
        const document1 = clone.ownerDocument;
        ...
        const anonymousReplacedElement = document1.createElement('html2canvaspseudoelement');
        ...
        declaration.content.forEach((token)=>{
            if (token.type === 0 /* TokenType.STRING_TOKEN */ ) {
                ...
                const img = document1.createElement('img');
                img.src = token.value;
                img.style.opacity = '1';
                ...
                if (token.name === 'attr') {
                    ...
                    if (counter && isIdentToken(counter)) {
                        ...
                        const counterType = counterStyle && isIdentToken(counterStyle) ? listStyleType.parse(this.context, counterStyle.value) : 3 /* LIST_STYLE_TYPE.DECIMAL */ ;
                ...
                switch(token.value){
                    case 'open-quote':
                        anonymousReplacedElement.appendChild(document1.createTextNode(getQuote(declaration.quotes, this.quoteDepth++, true)));
                        break;
                    case 'close-quote':
                        anonymousReplacedElement.appendChild(document1.createTextNode(getQuote(declaration.quotes, --this.quoteDepth, false)));
                        break;
                    default:
...
const createStyles = (body, styles)=>{
    const document1 = body.ownerDocument;
    if (document1) {
...
class Cache {
    ...
    async loadImage(key) {
        const isSameOrigin = typeof this._options.customIsSameOrigin === 'function' ? await this._options.customIsSameOrigin(key, CacheStorage.isSameOrigin) : CacheStorage.isSameOrigin(key);
...
const calculateBackgroundSize = (size, [intrinsicWidth, intrinsicHeight, intrinsicProportion], bounds)=>{
    ...
    if (isAuto(first) && (!second || isAuto(second))) {
        ...
        if (hasIntrinsicDimensions && hasIntrinsicProportion) {
            const width = hasIntrinsicWidth ? intrinsicWidth : intrinsicHeight * intrinsicProportion;
            const height = hasIntrinsicHeight ? intrinsicHeight : intrinsicWidth / intrinsicProportion;
            return [
                width,
                height
            ];
    ...
    async renderBackgroundImage(container) {
        ...
        for (const backgroundImage of container.styles.backgroundImage.slice(0).reverse()){
            if (backgroundImage.type === 0 /* CSSImageType.URL */ ) {
                ...
                const position = backgroundImage.position.length === 0 ? [
                    FIFTY_PERCENT
...
const iOSBrokenFonts = [
    '-apple-system',
    'system-ui'
];
const fixIOSSystemFonts = (fontFamilies)=>{
    return /iPhone OS 15_(0|1)/.test(window.navigator.userAgent) ? fontFamilies.filter((fontFamily)=>iOSBrokenFonts.indexOf(fontFamily) === -1) : fontFamilies;
};
...
const parseBackgroundColor = (context, element, backgroundColorOverride)=>{
    ...
    const defaultBackgroundColor = typeof backgroundColorOverride === 'string' ? parseColor(context, backgroundColorOverride) : backgroundColorOverride === null ? COLORS.TRANSPARENT : 0xffffffff;
...
//# sourceMappingURL=node_modules_c73aef4e._.js.map