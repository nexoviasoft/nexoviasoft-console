...
} from "@/api/admin/documents/documentsApi";
...
export default function LetterBuilder({ template, onBack, documentId }) {
  ...
  const [updateDocument, { isLoading: isUpdating }] =
    useUpdateDocumentMutation();
  const [sendDocumentByEmail, { isLoading: isSending }] =
    useSendDocumentByEmailMutation();
    ...
    if (documentId && documentData?.data) {
      ...
      if (doc.data && doc.type === "letter") {
        setData({
          ...
          role: doc.data.role || "",
          startDate:
            doc.data.startDate || new Date().toISOString().split("T")[0],
  ...
  const handleSendEmail = async () => {
    ...
    const recipientName =
      emailData.recipientName || data.candidateName || "Recipient";
        ...
        <div className="glass-card border-white/20 rounded-lg p-6 md:p-12 min-h-[500px] md:min-h-[600px] text-sm text-white print:shadow-none print:border-none print:w-full print:absolute print:top-0 print:left-0 print:z-50 w-full mx-auto print:mx-0 font-serif leading-relaxed overflow-x-auto">
          <div className="min-w-[600px] md:min-w-0">
            ...
            <div className="text-center mb-8 border-b border-white/20 pb-6">
              <div className="text-2xl font-bold text-black tracking-widest mb-2">
                NEXOVIASOFT INC.
              </div>
            ...
            <div className="mb-8">
              ...
              <div className="text-white/80">
                Rangpur City, Rangpur Division — 5400
              </div>
            </div>

            <div className="text-center font-bold mb-8 uppercase tracking-wider text-base text-black">
              {getLetterTitle()}
            </div>
            ...
            <div className="space-y-4 text-justify text-white/90">
              ...
              {template === "offer-letter" && (
                <>
                  <p>
                    We are pleased to extend an offer for you to join{" "}
                    <strong className="text-white">NexoviaSoft Inc.</strong> in
                    the position of{" "}
                    <strong className="text-white">
                      {data.role || "[Role]"}
                    </strong>
                    . We were impressed with your skills and experience and
                    believe you will be a valuable asset to our team.
                  </p>
                  <p>
                    Your starting annual salary will be{" "}
                    <strong className="text-white">
                      {data.salary || "[Salary]"}
                    </strong>
                    , along with our standard benefits package. You will be
                    reporting to{" "}
                    <strong className="text-white">
              ...
              {template === "appointment-letter" && (
                <>
                  <p>
                    Further to our recent discussions and your acceptance of our
                    offer, we are delighted to confirm your appointment as{" "}
                    <strong className="text-white">
                      {data.role || "[Role]"}
                    </strong>{" "}
                    at <strong className="text-white">NexoviaSoft Inc.</strong>,
                    effective from{" "}
                    <strong className="text-white">{data.startDate}</strong>.
                  </p>
                  <p>
                    Your annual compensation package is fixed at{" "}
                    <strong className="text-white">
                      {data.salary || "[Salary]"}
                    </strong>
                    . The terms and conditions of your employment are outlined
                    in the attached Employee Handbook.
                  </p>
                </>
              )}

              <p>
                We look forward to welcoming you to the NexoviaSoft family.
                Please sign and return a copy of this letter to acknowledge your
                acceptance.
              </p>
            </div>
            ...
            <div className="mt-16">
              <div className="grid grid-cols-2 gap-12">
                <div>
                  {/* Signature */}
                  <div className="text-3xl text-cyan-400 font-[cursive] mt-3">
                    NexoviaSoft
                  </div>
                  <div className="border-t border-white/20 w-3/4 pt-2 font-bold text-white">
      ...
      <Dialog open={isEmailDialogOpen} onOpenChange={setIsEmailDialogOpen}>
        <DialogContent className="glass-panel border-white/20 text-white max-w-md">
          ...
          <div className="space-y-4 mt-4">
                  ...
                  <SelectContent className="bg-black/90 border-white/20 text-white max-h-[200px]">
                    {teamMembers.map((member) => (
                      <SelectItem
                        ...
                        value={String(member.id || member._id)}
                      >
            ...
            {(emailData.selectedUserType === "manual" ||
              !emailData.selectedUserType) && (
              <div className="space-y-2">
