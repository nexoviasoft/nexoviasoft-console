...
const Header = ({ title = "NexoviaSoft", subtitle, onMenuClick }) => {
  ...
  return (
    <header className="relative bg-white/5 backdrop-blur-xl border-b border-white/10 px-4 lg:px-8 py-4 lg:py-6 text-white">
      <div className="absolute inset-y-0 left-0 w-1 bg-[#F58220] rounded-r-full shadow-[0_0_18px_rgba(245,130,32,0.6)]" />
      <div className="absolute inset-y-0 right-0 w-1 bg-[#F58220] rounded-l-full shadow-[0_0_18px_rgba(245,130,32,0.6)]" />
      <div className="flex items-center lg:items-start justify-between mb-0 lg:mb-4 relative z-10 gap-4">
        <div className="flex items-center gap-4">
          <button 
            onClick={onMenuClick}
            className="lg:hidden p-2 -ml-2 text-white/70 hover:text-[#F58220] transition-colors"
          >
        ...
        <div className="flex items-center gap-2 lg:gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 px-4 py-2 border border-white/20 rounded-lg bg-white/5">
            <button className="text-[#F58220] hover:brightness-110">
              <ChevronLeft className="w-4 h-4 text-[#F58220]" />
            </button>
          ...
          <button className="p-2 text-[#F58220] hover:bg-white/10 rounded-lg">
            <Search className="w-5 h-5 text-[#F58220]" />
          </button>
          <button className="relative p-2 text-[#F58220] hover:bg-white/10 rounded-lg">
            <Bell className="w-5 h-5 text-[#F58220]" />
            <span className="absolute top-1 right-1 w-2 h-2 bg-[#F58220] rounded-full shadow-[0_0_12px_rgba(245,130,32,0.8)]"></span>
          </button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="p-2 text-[#F58220] hover:bg-white/10 rounded-lg transition-colors">
                <Avatar className="w-8 h-8 border border-[#F58220]/30">
                  {user?.profileImage ? (
                    <AvatarImage src={user.profileImage} alt={getUserName()} />
                  ) : null}
                  <AvatarFallback className="bg-[#F58220]/20 text-[#F58220] text-sm font-semibold">
                    {getUserInitials()}
                  </AvatarFallback>
                </Avatar>
              </button>
              ...
              <DropdownMenuLabel className="px-3 py-2">
                <div className="flex items-center gap-3">
                  <Avatar className="w-10 h-10 border border-white/20">
                    ...
                    <AvatarFallback className="bg-[#F58220]/20 text-[#F58220] font-semibold">
                      {getUserInitials()}
                    </AvatarFallback>
                  </Avatar>
